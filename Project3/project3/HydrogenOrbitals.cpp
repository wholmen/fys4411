...
double HydrogenOrbitals::psi2p(vec R,double r, double d){
    //if (d == 0) return R(0)*(alpha*r*exp(-alpha*r/2.0));
    if (d == 0 && ml == -1) return R(0)*(alpha*r*exp(-alpha*r/2.0));
    if (d == 0 && ml ==  0) return R(1)*(alpha*r*exp(-alpha*r/2.0));
    if (d == 0 && ml ==  1) return R(2)*(alpha*r*exp(-alpha*r/2.0));
...
double HydrogenOrbitals::psi(double r, vec R){
...
double HydrogenOrbitals::dpsi(double r, vec R){
...
double HydrogenOrbitals::d2psi(double r, vec R){
